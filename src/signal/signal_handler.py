...
class SignalHandler():
    ...
    def signal_init(cls, ui):
        ...
        cls.ui.BassGuitarSelect.clicked.connect(lambda : cls.change_instrument(cls.ui.BassGuitarBack, INSTRUMENTS_THEME["bass_guitar"]))
        cls.ui.OrganSelect.clicked.connect(lambda : cls.change_instrument(cls.ui.OrganBack, INSTRUMENTS_THEME["organ"]))
        cls.ui.AccordionSelect.clicked.connect(lambda : cls.change_instrument(cls.ui.AccordionBack, INSTRUMENTS_THEME["accordion"]))
        cls.ui.PianoSelect.clicked.connect(lambda : cls.change_instrument(cls.ui.PianoBack, INSTRUMENTS_THEME["piano"]))
        cls.ui.HarpSelect.clicked.connect(lambda : cls.change_instrument(cls.ui.HarpBack, INSTRUMENTS_THEME["harp"]))
        cls.ui.VibraPhoneSelect.clicked.connect(lambda : cls.change_instrument(cls.ui.VibraPhoneBack, INSTRUMENTS_THEME["vibra_phone"]))

        cls.ui.QuitButton.clicked.connect(sys.exit)

        cls.ui.BlueToothButton.clicked.connect(lambda : cls.change_page(BLUETOOTH_PAGE_INDEX))
        cls.ui.InstrumentButton.clicked.connect(lambda : cls.change_page(INSTRUMENT_PAGE_INDEX))
        cls.ui.SettingButton.clicked.connect(lambda : cls.change_page(SETTING_PAGE_INDEX))
